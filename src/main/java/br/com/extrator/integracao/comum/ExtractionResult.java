...
public class ExtractionResult {
    ...
    public static Builder bloqueadoPorDependencia(final String entityName,
                                                  final LocalDateTime inicio,
                                                  final String dependencia,
                                                  final String detalhe) {
        final String mensagem = "Bloqueado por dependencia: "
            + dependencia
            + (detalhe == null || detalhe.isBlank() ? "" : " | " + detalhe);
        return new Builder(entityName, inicio)
            .status(ConstantesEntidades.STATUS_INCOMPLETO_DADOS)
            .registrosSalvos(0)
